    ...
    <script type="module">
            ...
            return `
                <div class="${size === 'modal' ? 'mb-6' : 'mb-1.5'}">
                    <div class="flex items-baseline flex-nowrap gap-1.5 sm:gap-2 overflow-hidden">
                        ${hasCurrent ? `<span class="font-serif ${currentSize} text-[#0b2e22] font-bold leading-none tracking-tight tabular-nums whitespace-nowrap">৳${window.formatMoney(current)}</span>` : ''}
                        ${hasOld ? `<span class="${oldSize} text-gray-400 line-through font-medium leading-none tabular-nums whitespace-nowrap">৳${window.formatMoney(old)}</span>` : ''}
                    </div>
            ...
            return reviews.map(r => `
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 hover:shadow-xl transition-shadow relative">
                    ...
                    <div class="flex items-center gap-4 mt-auto">
                        <div class="w-12 h-12 bg-gray-200 rounded-full overflow-hidden border border-gray-100"><img src="${r.avatar || r.image || 'https://i.pravatar.cc/150'}" onerror="this.src='https://i.pravatar.cc/150'" class="w-full h-full object-cover"></div>
                        <div>
                    ...
                    ${displayedBouquets.map((bouquet, index) => `
                        <div class="group cursor-pointer flex flex-col animate-fade-up bg-white rounded-[1.5rem] p-2.5 sm:p-3 shadow-sm hover:shadow-2xl transition-all duration-400 border border-gray-100 hover:border-[#c8a13a]/30" style="animation-delay: ${Math.min(index, 8) * 0.04}s" onclick="openDetailModal('${bouquet.id}')">
                            ...
                            <div class="px-1.5 pb-1 flex flex-col flex-1 justify-between">
                                <div>
                                    <h3 class="text-[13px] sm:text-base font-serif text-gray-900 mb-1 group-hover:text-[#10583f] transition-colors leading-snug">${bouquet.name || 'Fresh Bouquet'}</h3>
        ...
        window.renderHome = function() {
            ...
            container.innerHTML = `
                ...
                <section class="relative pt-24 pb-12 md:pt-36 md:pb-20 px-6 overflow-hidden">
