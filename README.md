...
<body class="text-gray-800 antialiased selection:bg-[#10583f] selection:text-white">
    ...
    <script type="module">
        ...
        window.state.visibleCount = BOUQUETS_PAGE_SIZE;
        ...
        // Deterministic hash so the same bouquet ID always produces the same
        // code (stable across reloads/devices), but different bouquets get
        // different codes even if their names or IDs look similar.
        window.hashToCode = function(input) {
            ...
            return combined.toString(36).toUpperCase().padStart(7, '0').slice(-6);
        };

        // Every bouquet gets its own unique, findable code — "TF-XXXXXX".
        // If an admin has explicitly set a custom secretCode on the bouquet,
        // that takes priority; otherwise one is generated automatically from
        // the bouquet's unique ID, so it's the same every time and works for
        // both existing and newly added bouquets without any extra setup.
        window.getSecretCode = function(bouquet) {
            if (bouquet.secretCode) return bouquet.secretCode;
            const seed = bouquet.id || bouquet.name || Math.random();
            return `TF-${window.hashToCode(seed)}`;
        };
        ...
        window.renderReviewCardHTML = function(r) {
            return `
                <div class="review-card bg-white p-7 sm:p-8 rounded-[2rem] shadow-sm border border-gray-100 hover:shadow-xl transition-shadow relative flex flex-col">
                    ...
                    <div class="flex items-center gap-4 mt-auto">
                        <div class="w-12 h-12 bg-gray-200 rounded-full overflow-hidden border border-gray-100 flex-shrink-0"><img src="${window.secureUrl(r.avatar || r.image || 'https://i.pravatar.cc/150')}" onerror="this.src='https://i.pravatar.cc/150'" class="w-full h-full object-cover"></div>
                        <div>
        ...
        window.renderReviewsHTML = function() {
            ...
            const cards = reviews.map(r => window.renderReviewCardHTML(r)).join('');
            // Duplicate the set once so the loop from 0 to -50% is seamless,
            // and scale the animation duration to the amount of content so
            // the scroll speed feels consistent regardless of review count.
            const duration = Math.max(reviews.length * 7, 18);
            return `
        ...
        window.computeFilteredBouquets = function() {
            ...
            if (q) {
                ...
                list = list.filter(b => {
                    const code = window.getSecretCode(b).toLowerCase();
                    const codeFlat = code.replace(/[\s-]/g, '');
                    return (b.name || '').toLowerCase().includes(q) ||
                        (b.description || '').toLowerCase().includes(q) ||
                        code.includes(q) ||
                        (qFlat.length > 0 && codeFlat.includes(qFlat));
                });
            }
            return window.sortByPosition(list);
        };

        window.renderBouquetGridHTML = function(filteredBouquets) {
            ...
            const visibleCount = window.state.visibleCount || BOUQUETS_PAGE_SIZE;
            const displayedBouquets = filteredBouquets.slice(0, visibleCount);
            const remaining = filteredBouquets.length - displayedBouquets.length;
            return `
                ...
                <div class="flex flex-col items-center gap-4 sm:gap-5 mt-10 sm:mt-14">
                    <p class="text-xs font-semibold tracking-[0.15em] uppercase text-gray-400">
                        Showing ${displayedBouquets.length} of ${filteredBouquets.length} designs
                    </p>
                    ${remaining > 0 ? `
                        <button onclick="loadMoreBouquets()" class="group inline-flex items-center gap-3 premium-btn text-white px-10 sm:px-12 py-4 sm:py-5 rounded-full text-xs sm:text-sm uppercase tracking-[0.2em] font-bold">
                            See More
                            <svg class="w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-y-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                        </button>
                    ` : ''}
                </div>
            `;
        ...
        window.loadMoreBouquets = function() {
            window.state.visibleCount = (window.state.visibleCount || BOUQUETS_PAGE_SIZE) + BOUQUETS_PAGE_SIZE;
            ...
            container.innerHTML = `
                ...
                <section class="py-20 bg-[#faf8f4] border-t border-[#c8a13a]/10 relative overflow-hidden">
                    ...
                    <div class="max-w-7xl mx-auto px-6 text-center relative z-10">
                        ...
                        <div class="text-left" id="reviews-grid-wrapper">
