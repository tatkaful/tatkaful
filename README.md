...
<body class="text-gray-800 antialiased selection:bg-[#10583f] selection:text-white">
    ...
    <script type="module">
        ...
        const DEFAULT_STATE = {
            ...
            bouquets: [
                { id: '1', name: 'Royal Crimson', description: 'Deep red premium roses hand-tied with elegant greenery.', images: ['https://images.unsplash.com/photo-1561181286-d3fee7d55364?auto=format&fit=crop&w=800&q=80'], bestseller: true, visible: true, currentPrice: '1200', oldPrice: '1500', position: 1 },
                { id: '2', name: 'Morning Dew', description: 'Fresh white lilies and soft pink tulips.', images: ['https://images.unsplash.com/photo-1526047932273-341f2a7631f9?auto=format&fit=crop&w=800&q=80'], bestseller: false, visible: true, currentPrice: '850', oldPrice: '1000', position: 2 }
            ],
            reviews: [
                { id: 'r1', name: 'Nusrat Jahan', stars: 5, text: 'Absolutely loved the bouquet, fresh and elegant. The delivery was right on time!', image: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80' }
            ],
            settings: {
        ...
        // Cache strategy for instant 1-second load in social webviews
        ...
        onValue(dataRef, (snapshot) => {
            ...
            if (val) {
                if (val.bouquets) window.state.bouquets = val.bouquets;
                if (val.reviews) window.state.reviews = val.reviews;
                if (val.settings) window.state.settings = { ...DEFAULT_STATE.settings, ...val.settings };
                
                // Cache locally for next instant load
            ...
            console.warn("Firebase live sync fallback to local cache:", err);
        });
        ...
        window.orderViaWhatsApp = function(bouquetId, event) {
            ...
            if (bouquet.images && bouquet.images[0] && !bouquet.images[0].startsWith('data:')) {
                try {
                    ...
                    console.error('Auto-download image:', err);
                }
            }
        ...
        window.renderReviewsSectionHTML = function() {
            if (!window.state.reviews || window.state.reviews.length === 0) return '';
            return `
                <section class="px-6 py-20 sm:py-28 max-w-7xl mx-auto">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                        ${window.state.reviews.map((r, i) => `
                            <div class="luxury-card rounded-[2rem] p-8 sm:p-10 flex flex-col animate-fade-up" style="animation-delay:${Math.min(i, 8) * 0.05}s">
                                ...
                                <div class="flex gap-1 mb-4">${window.starGlyphs(r.stars || 5)}</div>
                                ...
                                <div class="mt-auto flex items-center gap-4 pt-4 border-t border-gray-50">
                                    <img src="${r.image || window.state.settings.logo}" onerror="this.src='https://placehold.co/100x100/10583f/ffffff'" class="w-12 h-12 rounded-full object-cover border border-gray-100" />
        ...
        window.renderHome = function() {
            ...
            container.innerHTML = `
                <nav class="fixed w-full top-0 z-40 glass-header">
                    <div class="max-w-7xl mx-auto px-6 h-20 md:h-24 flex items-center justify-between">
                        <div class="flex items-center gap-3.5 cursor-pointer group" onclick="window.scrollTo({top:0, behavior:'smooth'})">
                            <img src="${window.state.settings.logo}" onerror="this.src='https://placehold.co/100x100/10583f/ffffff'" alt="Tatka Ful Logo" class="w-12 h-12 md:w-14 md:h-14 rounded-full object-cover shadow-sm group-hover:scale-105 transition-transform duration-300" />
                        ...
                        <div class="hidden md:flex items-center gap-10 text-sm font-semibold tracking-widest uppercase text-gray-500">
                            ...
                            <a href="https://wa.me/${window.formatWhatsApp(window.state.settings.whatsapp)}" target="_blank" rel="noreferrer" class="premium-btn text-white px-8 py-3.5 rounded-full tracking-widest text-xs font-bold uppercase">
                                Inquire Now
                            </a>
                        </div>
                    </div>
                ...
                ${window.renderReviewsSectionHTML()}
                ...
                <footer class="bg-white border-t border-gray-100 pt-20 sm:pt-24 pb-12 px-6">
                    <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12 text-center md:text-left">
                        <div class="flex flex-col items-center md:items-start max-w-sm">
                            <img src="${window.state.settings.logo}" onerror="this.src='https://placehold.co/100x100/10583f/ffffff'" alt="Logo" class="w-20 h-20 rounded-full mb-6 shadow-sm object-cover" />
